...
describe('SessionCustom (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const mockSessionCustomService = {
      ...
      createSessionCustom: jest.fn().mockImplementation(() =>
        Promise.resolve({
      ...
      removeSessionCustom: jest.fn(),
      addPoseToSessionCustom: jest.fn(),
      removePoseFromSessionCustom: jest.fn(),
      updateSessionCustom: jest
        .fn()
  ...
  it('/session-custom/1 (PATCH)', () => {
    return request(app.getHttpServer())
      .patch('/session-custom/1')
      .auth(accessToken, { type: 'bearer' })
      .send({
        title: 'Jambes lourdes',
      })
      .expect(200)
      .then((response) => {
        expect(response.body.title).toEqual('Jambes lourdes');
      });
  });

  it('/session-custom/1 (PATCH) - failed', () => {
    return request(app.getHttpServer())
      .patch('/session-custom/99')
      .auth(accessToken, { type: 'bearer' })
      .send({
        title: 'Jambes lourdes',
      })
  ...
  it('/session-custom/1 (DELETE)', () => {
    return request(app.getHttpServer())
      .delete('/session-custom/1')
      .auth(accessToken, { type: 'bearer' })
      .expect(200);
  });

  it('/session-custom/1/poses/4 (POST)', () => {
    return request(app.getHttpServer())
      .post('/session-custom/1/poses/4')
      .auth(accessToken, { type: 'bearer' })
      .expect(201);
  });

  it('/session-custom/1/poses/4 (DELETE)', () => {
    return request(app.getHttpServer())
      .delete('/session-custom/1/poses/4')
      .auth(accessToken, { type: 'bearer' })
      .expect(200);
  });
});
